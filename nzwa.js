	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '558287369797@s.whatsapp.net'
		...
		nzwa.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               nzwa.sendMessage(from, 'Número Da Minha Dona, Caso Tenha Alguma Dúvida Chama Ela',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                nzwa.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
